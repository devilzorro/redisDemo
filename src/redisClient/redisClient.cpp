...
bool CRedisClient::HSet(const std::string &hkey, const std::string &key, const std::string &value) {
	if (!m_connect)
	{
		return false;
	}

	m_reply = (redisReply*)redisCommand(m_connect, "HSET %s %s %s", (char *)hkey.c_str(), (char *)key.c_str(),(char *)value.c_str());
	...
	{
		printf("redis hset error, hkey = %s, key = %s, value = %s\n", hkey.c_str(),key.c_str(), value.c_str());
		FreeReply();
		return false;
	}
	else
	{
		printf("redis hset success,hkey = %s, key = %s, value = %s\n", hkey.c_str(),key.c_str(), value.c_str());
		FreeReply();
		return true;
	}
#else
